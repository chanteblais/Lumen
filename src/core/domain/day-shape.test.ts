...
import { shapeDay, spanWords } from "./day-shape";
...
describe("shapeDay", () => {
  ...
  it("assumes half an hour when there's no estimate, and drops landmarks already past", () => {
    ...
    expect(shape.segments).toEqual([{ kind: "window", label: "The rest of the afternoon", span: null, items: [item("a", null)] }]);
  });
  ...
  it("spills what can't fit before the day ends, and calls a late window tonight", () => {
    ...
    expect(shape.segments).toEqual([{ kind: "window", label: "Tonight", span: null, items: [item("a", 60)] }]);
