"""Inline the room, the pieces, rig-walk.json, morph.json and room-home.json into index.src.html -> index.html.

    python3 art/prototypes/lumi-walk/build.py

Run split.py, then facings.py, then morph.py first. index.html is the file that is published (and the one capture.py
renders).
"""
import base64
import json
...
for token in ('/*RIG*/', '/*MORPH*/', '/*ROOM*/', '/*PARTS*/', '__ROOM_URI__'):
