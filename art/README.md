...
| A gesture widens the figure on one side (the waving hand), so centring on the body slides her | cells placed by the edge that holds — the hood's right edge — and settled on the head's other half (`hood_right`, `settle`) |
