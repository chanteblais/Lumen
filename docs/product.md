...
> **Partly superseded (2026-09-12).** This is the first brief, kept while the canon is assembled ([`PROJECT-CANON.md`](../PROJECT-CANON.md)). The philosophy now lives in [`philosophy/product-vision.md`](philosophy/product-vision.md). Two things here no longer hold ([`living/decisions.md`](living/decisions.md)): the visual direction (now inhabited, painted places, not "no decorative imagery") and the nav (now four places: Home · Garden · Library · Study). Whether rewards exist is contested ([open question 14](living/open-questions.md)). The pillars now live in the vision and the [Experience Principles](philosophy/experience-principles.md), and Lumi's voice in [`philosophy/lumi.md`](philosophy/lumi.md) (2026-09-13). This file is due for `docs/archive/` once the last references to it (the comment in `persona.ts`, the scenario list in `voice-eval-log.md`) point there.
