...
## 2026-09-18 · Today is the day at a glance; there is no Right now card

**Decision.** Today shows the day at a glance, after Chanté's mockup (`art/mockups/today-at-a-glance-mockup.webp`, "a general idea"): a card per area of life (each list with anything open) saying in a few words how it stands today; *Suggested for today*, the path as one short list in Lumi's order with every row alike; what's already done today; *Everything else can wait*; and *Up next*, the fixed times. There is no Right now card. Turning something down and breaking it into steps happen on any row, opened in place; turning down a thing further down the list just takes it off today.
**Rationale.** Chanté (2026-09-18): "Could you update today to focus less on breaking down a single task, and more to give you today at a glance?" Then, on the first version (one sheet with Right now leading it), with the mockup: "Let's get rid of this right now item. Instead it should be more of a Today at a Glance." *(Claude's reading:)* the page had become a workspace for one task when what she wants from it is orientation: what today holds across her life, and what's suggested. The curation stays: the path is still Lumi's short, ordered cut, nothing past it is listed, and each area that has nothing for today says it can wait.
**Implications.**
- **Contradicts settled canon, not yet reconciled:** [`today-garden.md`](../product/today-garden.md) §4 (*one thing should dominate*) and §5's strict one-second test, [`today.md`](../today.md) principle 2, and canon principle 4's *never build a dashboard* — area cards are the most dashboard-like thing in the product. How the canon changes (or whether the page later moves back) is Chanté's call, on the [desk](../desk/README.md). Until she answers, the page stays as built.
- **Taken from the mockup:** the area cards, Suggested for today as a list with circles, tags, estimates and chevrons, Up next as a line of times. **Left out, by settled decisions:** the energy/mood/focus sliders (the user never rates anything; capacity stays a question in words), *Your rhythms* and its dots (a streak), quotes and taglines (no inspirational copy), counts such as "2 items due soon" (*never a count of what's undone*), *Let's do this together* (focus sessions are set aside). **Left out, Claude's call:** *Adjust priorities* (said to Lumi), the List/Timeline switch, *Add something else* (chat is capture), *See calendar* (no calendar in V1), Lumi's greeting in a speech bubble (a bubble is where she answers; she never speaks unprompted).
- The area cards are derived at read time from what the user already said (priorities), dates and the path; nothing new is stored or asked. *Done today* is titles only, oldest first, never a count ([burden log](../ef-burden-log.md): nothing asked of the user).
- The plan still names a first thing (`rightNow`) — it orders the list and is what a chat ask pins — so moving back toward one dominant thing later needs no planner change.
**Principle.** Show less than you know (the path and each area's line are curated, not listed); correction beats configuration (*Not this* on any row); no methodology to learn.
