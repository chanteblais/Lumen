...
## 2026-09-18 · Break it down leaves Today; breaking things down stays in the conversation

**Decision.** Today no longer offers *Break it down*, and no longer shows a *First* step under the thing to start. Each row is the thing, its list and size, a circle to tick it and *Not this*. Lumi still breaks something down when it genuinely feels too big — in conversation, one question first (persona → *Starting*), and the *Break it down* quick start on Home stays.
**Rationale.** Chanté (2026-09-18): "Can you get rid of the breakdown feature? 'Call the dentist' isn't made any easier by breaking it down into 'pull up your dentist's phone number'." *(Claude's reading:)* the feature assumed every thing on the path was a threshold problem. Most aren't: a call, an errand, a reply. For those, a first step is a line to read and a breakdown is a choice among steps — more to process, no lower threshold. The canon's *starting is its own problem* (V§6, EP§10) is about the cases where it is, and Lumi can tell those apart in conversation; a button on every row can't.
**Implications.**
- `core/ai/breakdown.ts`, `POST /api/intentions/[id]/steps`, the `first_step` PATCH action and `setFirstStepInPlan` are removed. The plan still carries `rightNow.firstStep` — the planner writes it and `reshape_today` pins it — for chat; the page doesn't show it.
- The row loses its chevron: *Not this* is a quiet word at the end of every row, its chips opening underneath.
- Garden §19 (*Today should support initiation*) is now served by the order, *Start here* and the conversation, not by a step on the page. If living with it shows the first step was doing real work for someone, it comes back as one line, not as a breakdown.
**Principle.** Show less than you know; one decision at a time; starting is its own problem (met where it is one).
**Replaces.** The *Break it down* half of *Today's card does its own organising* (2026-09-13), and the *First* line of the spine cut earlier on 2026-09-18.

## 2026-09-18 · Rhythms: the routines they're building are held in their words, and Today shows when each happened

**Decision.** A **rhythm** is a first-class thing: a routine the user says they're building ("go to the gym more", "meditate every morning"), held in their words, with how often in their words if they said, kept apart from tasks, beliefs and priorities. Lumi holds one when they say it (`hold_rhythm`), never files it as a task, records the day it happened when they tell her (`practiced_rhythm`), and lets it go when they're done. Today shows each rhythm as its name, the week as seven marks with a fill on the days it happened, their words, and the same circle every row has, for today, that records or takes back today (it began as a *Did it* word and became the circle the same day, at her ask for something more intuitive). The day planner may say there's room for one ("There's room for the gym before practicum") and never more. **Never a target, a streak, a tally, or a missed day.**
...
- The circle's tap is the one thing asked of the user, and only because nobody can derive a gym visit. It is logged in the [burden log](../ef-burden-log.md) with the guards that keep it from becoming a tracker: nothing happens if it's never tapped, and Lumi never asks.
...
**Rationale.** Chanté (2026-09-18): "focus less on breaking down a single task, and more to give you today at a glance"; then, with her mockup (`art/mockups/today-at-a-glance-mockup.webp`), "Let's get rid of this right now item"; then "take some creative license on how to implement this page in a way that might help someone orient their day, particularly someone with ADHD-like traits … take into consideration the project's design philosophy." *(Claude's reading, and the design's reasons:)* orientation for those traits means answering, without a tap, *how much day is actually left and when the fixed thing is* (time blindness — said in words, per §17), *what fits before I have to leave* (transitions, §58, §93), *where do I start* (initiation: the first physical step in view, §19), *what have I already done* (return without shame: the past of the day in its place, §22), and *is everything else being held* (containment per area, §7–8), while choosing nothing among equals (Lumi's order, one *Start here*). A flat list with area boxes (the second cut) answered only the last two; a single big card (the first) answered only the third.
**Implications.**
- **Tension with settled canon, reduced but not gone:** [`today-garden.md`](../product/today-garden.md) §4 says *one thing should dominate*; on the spine one thing **leads** (*Start here*, on a wash, step open) but does not take the page. §5's one-second test is answered as *Now → Start here*. Canon principle 4's *never build a dashboard*: the area lines are the nearest thing, and are kept to one line each with no boxes and no numbers. Whether §4 becomes "one thing leads" is Chanté's call, on the [desk](../desk/README.md). Until she answers, the page stays as built.
- **From the mockup:** the sense of the day's areas, a list with circles, tags, estimates and chevrons, the fixed times as a line. **Left out, by settled decisions:** the energy/mood/focus sliders (the user never rates anything; capacity stays a question in words), *Your rhythms* and its dots (a streak), quotes and taglines (no inspirational copy), counts such as "2 items due soon" (*never a count of what's undone*), *Let's do this together* (focus sessions are set aside). **Left out, Claude's call:** area cards as boxes (a dashboard), *Adjust priorities* (said to Lumi), the List/Timeline switch (the spine is one view, neither), *Add something else* (chat is capture), *See calendar* (no calendar in V1), Lumi's greeting in a bubble (a bubble is where she answers; she never speaks unprompted).
- Everything on the sheet is derived at read time from what's already held (the plan, dates, estimates, priorities, completion times, the clock); nothing new is stored or asked ([burden log](../ef-burden-log.md): nothing asked of the user). The layout is recomputed from the clock on every open (§57), but the plan itself still moves only on its triggers ([open question 26](open-questions.md)).
- The plan still names a first thing (`rightNow`) — it is *Start here*, and what a chat ask pins — so moving back toward one dominant thing later needs no planner change.
**Principle.** Show less than you know (the path and each area's line are curated, not listed; time in words); starting is its own problem (the step is in view); design for return (the day so far, above Now); correction beats configuration (*Not this* on any row).
