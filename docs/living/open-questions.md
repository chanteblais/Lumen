...
## 29. How much of a dashboard should Today be?

**Status: Decided in part 2026-09-18; the rest open.** Chanté chose the first and last of the candidates below ("I think 1 and 5 is good"): *how full the day is* as a shape, and the line — **state and shape yes, counts and scores no** — written into principle 4, EP§4's *watch for*, the Garden's §156 and the drift list ([decision](decisions.md#2026-09-18--state-and-shape-at-a-glance-are-allowed-counts-and-scores-are-not)). She also asked for a **rhythms** section, which the same decision covers. Still open: how the week stands (candidate 2), things resting a while (3), and whether any number belongs (4). Her words, 2026-09-18, after Today became the rest of the day laid along itself: "I'm not actually sure about the 'never a dashboard'. I'm starting to feel like it should be a bit of a dashboard." The canon says the opposite in several places: principle 4 (*never build a dashboard*, V§11, EP§4), EP§4's *watch for* list (dashboards, widget accumulation, badge counts, dense metadata, competing calls to action), the Garden's §156 (*not a dashboard, not a productivity scorecard*) and the drift list (*the Garden becoming a task dashboard*). None of those is changed by this entry.

*(Claude's reading, for her to correct:)* the canon's objection was never to seeing the state of one's day at a glance — it was to two things: **metrics** (counts, progress, streaks, scores: things that judge) and **the user doing the reading** (many equal widgets, so the prioritising is handed back). The page as built already shows state at a glance without either: time in words, one line per area with no numbers, Lumi's order. So the question is what "a bit more" would add. Candidates, each of which the canon currently forbids or discourages: a sense of how full the day is (a shape, not a percentage); how the week stands, not just today; what's been resting a while (§55 says repeated deferral is a signal, not a score); something numeric she'd actually want, if any. Decide: which of these, and the line — the working proposal is *state and shape are allowed; counts, scores and anything the user must interpret are not*. When decided, principle 4, EP§4's *watch for*, Garden §156 and the drift list change together, with the reason.

Related: [Today / Garden](../product/today-garden.md) §4 (retired), §156; [decision, 2026-09-18](decisions.md); [`today.md`](../today.md).
