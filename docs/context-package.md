...
5. Capture changed UI from a build of the verified main baseline. Replace affected image bytes under the same IDs below; do not create accumulating dated duplicates. Current captures are actual interface images, not generated mockups.

**Chanté said go, and the blocker is not her permission (2026-09-14).** On the [Coherence Desk](desk/README.md), round 1, she answered *Go with that* to refreshing this package in place now and again after any landed change to what a place does. The ask was framed as though Claude could then do it; it can't. The tooling limit below still stands, re-verified 2026-09-14 against the connector's own schema (`update_file` accepts `fileId`, `title` and `parentId` — no content). So her yes stands as **standing permission to write to her Drive for this package**, and what is missing is a write path, not an answer. Until there is one, a refresh is: Claude prepares the replacement text for each affected document from `main` and hands it over; she pastes it in and uploads new image versions. That is the shape it takes on the desk from round 2 — her hands, not an ask.

**Tooling limit (found 2026-09-14, refresh toward `2cf5dc3` not applied).** The Drive connector cannot write this package: `update_file` changes only a file's title and parent, and `create_file` would make duplicates. A refresh therefore needs another write path, such as Chanté pasting prepared text into each doc and uploading new image versions in Drive (right-click → File information → Manage versions), or a Docs-capable connector. Driving Google Docs through the automation browser was refused by the permission classifier. The automation Chrome window can also come up minimised (`document.visibilityState` hidden), which freezes Lumi in captures. Check both before starting.
