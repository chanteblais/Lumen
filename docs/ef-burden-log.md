...
|---|---|---|---|---|---|
| 2026-09-13 | Library dragging, `docs/spaces.md` §14 (proposed, not built) | set | Drag up and down for priority, between sections for context, to Later to stop foregrounding | Reopens the 2026-09-11 turn-down of drag-to-prioritise, this time as an optional correction, not upkeep. It holds only if Lumi orders and files everything without it, a drag is an event she derives from (no priority field to keep current), and nothing goes stale or looks wrong when the user never drags | open |
