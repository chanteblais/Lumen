...
├── scripts/                      gen-lumi-sheet.py (art/prompts/ → a generated sheet) · measure-lumi-sheet.py · draw-library-map.py (the Library's camera-view manifest → annotated maps) · cut-lumi-idle.py (art/ → public/) · cut-nav-art.py (Chanté's painted nav pieces, `art/ui/` → `public/nav-rail-*.webp`, `nav-parchment-*.webp`) · preview-lumi-loop.py · check-route-auth.mjs · preflight.mjs (before `check` and `dev`: installs, stale generated types, `.env.local` keys — `docs/dev-hygiene.md`) · voice-eval.mjs
├── drizzle.config.ts
├── .env.example
└── package.json
```
