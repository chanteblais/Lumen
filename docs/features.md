...
- **Every open feels fresh** (M4, Chanté's ask) — the transcript is in two parts: everything that was there when the page opened, then the greeting card, then what you say now. The page opens scrolled to the card, so Chat reads as new each time you come to it, and the earlier conversation is one scroll up, untouched. From your first message on, it follows the newest line like any chat. The card is full (with quick starts) after thirty minutes of quiet, compact within a sitting. The date rules between messages keep their six-hour window.
