...
| `.today-voice` | Lumi's words, on the painting: the running head (kicker + divider), the greeting (30px) and her day line (`.today-dayline`, 19px), then once a day the capacity question (`.today-capacity`, 18px) with its answers as `.tool-link` words parted by middle dots (*Skip* in the mute ink) — no chips. The clock's treatment: parchment tokens (`--ink #f7f1e5`) and a small dark text-shadow, over a feathered shadow (`::before`, `rgba(30,20,10,.5)` blurred 26px) so it reads on any part of the room. No portrait medallion — Lumi is standing in the room |
| `.today-now` | The one card, the only full paper on the page: card at .97 over a 20px blur, 10px radius, a hairline at .10, shadow .08 / .24. No kicker inside. The title (34px), the list · estimate note, *First* and the first step, then **Start with Lumi** (`.btn-primary`, 19px) and three `.tool-link` words — *Not this* · *Break it down* · *Done*. No circle in front of the title: the card invites starting (*Not this* still opens its chips in place) |
...
**Running heads:** every page but Chat opens with the same running head — its name as a `.label` kicker with a `Divider` beneath — and Insights then sets Lumi's 48px portrait as a `.medallion` beside the display-serif opening line, the way the greeting does on Chat. Today sets its opening line on the painting without the portrait (2026-09-13): Lumi is standing in the room. The browser tab reads the same way (`layout.tsx` title template): *Today · Coherence*, *Library · Coherence*; Home is just *Coherence*.

---

## Spacing & Layout

- **Shell:** CSS grid `calc(var(--rail-x) + var(--rail-w)) minmax(0,1fr)` — the rail is 64px wide, 10px in from the corner (48px and 6px on phones); the nav's parchment floats over the page and never takes a column, **viewport-height** (`.shell { height: 100dvh }`); main `.main` is a scrolling column (`overflow-y: auto`, padded `36px 48px 0`). The chat page overrides this with its own scroll region so the composer stays fixed.
- **Content width:** `max-w-[1080px]`, left-aligned within main.
- **Mobile breakpoint:** `767px` (CSS). The rail stays, at 48px; the parchment opens over the page when the compass star is tapped (a trial, 2026-09-13 — to revisit).
