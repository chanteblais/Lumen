...
The single source of Lumi's drawings. `public/lumi-heads.png`: one row of 176px square cells cut from the original `lumi.png` character sheet (not kept) — neutral · blink · happy · curious · excited · sleepy. `public/lumi-idle.webp`: a 14×9 grid of 144×208 cells, three rows per loop (open, half-shut and shut eyes — the blink frames composited on, so blinking and the loops run together): rows 0–2 the nine-frame **breath** loop and rows 3–5 the nine-frame **sway** loop from `art/lumi-slow-idle.png`, rows 6–8 the fourteen-frame **playful foot** loop from `art/lumi-idle.png` (shorter loops leave their trailing columns empty; `LUMI_LOOP_FRAMES` holds each loop's length). Every frame is the same pose at the same scale, centred on the figure and anchored on the feet baseline (the sheet's own spacing wobbles, so its offsets read as a slide); every loop starts and ends at the rest frame, so they hand over there. Cut by `scripts/cut-lumi-idle.py`: paper is flood-filled from outside the figure, so the cream hood survives the matte; on a contrasting ground (`contrast=True`, the foot sheet's grey-blue) the edge band is de-matted against the ground using the nearest interior pixel as the figure's own colour, and the shadow is read by its blue cast and redrawn in the slow-idle sheet's warm shadow tone (`SHADOW_RGB`), so nothing cold or grey rims her on the paper. The foot sheet is sixteen in-betweened frames of one drawing where only the foot and the flicked-up dirt move (the earlier eight-frame sheet redrew the whole figure each frame, so she boiled and the foot jumped); it is drawn larger, so its frames are scaled to 0.71 (hood top and width land on the breath loop's pixels), centred on the figure like the rest and then **settled on the head silhouette** — frame 0 onto the breath rest frame, the others onto frame 0; whole pixels sideways, then a quarter-pixel refinement in both axes (cubic resample on premultiplied channels), because a frame half a pixel off its neighbour doubles the hood's edge under the crossfade — and then **held**: every frame keeps frame 0's head, blended in over the collar rows on premultiplied channels (`hold_head`), because the generator still redrew the hood rim and its sun a little each frame and the crossfade showed it as a shimmer. The head holds still while the foot kicks, exactly. The sheet is two scuffs with a full return to rest between them; the cut takes frames 1–7 and 10–16 so the loop is one eased kick out and back. (Centring frames on the dark face instead had her hop ~14px on every hand-over, since her face sits right of her hood's centre.) `LumiSprite` draws one cell at a height via `background-position`; `headCell` / `idleCell` address them. A new state is a new cell, never a new component.
