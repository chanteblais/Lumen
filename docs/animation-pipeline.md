...
|---|---|---|---|---|---|---|---|
| 2026-09-13 | The wave (`lumi-wave.png`, 24 frames, 120 ms, a reaction on arrival) — tier 3 | 1 run of 4 candidates, generated by Claude (`gen-lumi-sheet.py`); 1 passed every gate | 1 — the sheet approved on first sight; the cut sent back once ("she jumps around a little bit") | ~1 h 40 (00:19 first run → the hold), incl. writing the generator and the Prompt lab | 2 scripts (+ the generator) + 2 components + 7 docs | The cut, not the sheet: a hand that widens her on one side broke centring on the body; settling on the head alone missed five cells dropped 2–4px (placed by the matte's lowest row) — found by measuring the held parts of the cut cells, fixed by aligning on them and holding everything but the arm; blink rows for 24 cells made the sheet 917 KB (dropped, then the hold: 461 KB) | the prompt was the bottleneck, not the generator; judge the sheet's motion as a GIF before the cut; measure the held parts of the *cut* cells before review; hold everything but the moving part; a short gesture needs no blink rows → Prompt lab findings, `art/README.md`, `decisions.md`, backlog #7 |
...
7. **Held parts in the preview.** The wave went to review with five cells dropped 2–4px because the cut's own check was head overlap alone; a throwaway script (feet, boots, lantern, hood edge and eyes per cut cell against the rest cell) found it in a minute. Fold that table into `preview-lumi-loop.py` with a pass line (every held part within 0.5px), so a jump is caught before Chanté sees one. *Saves* the round the wave spent.
