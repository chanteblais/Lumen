...
1. `src/components/chat/LumiSprite.tsx` — `LUMI_LOOPS` (the name), `LUMI_LOOP_CELLS` (the play order over the row's cells; repeats and reversals are free), `LUMI_LOOP_EYES` (the eye rows it has), `LUMI_IDLE_FRAMES` if the row is the widest yet; the header comment's row map.
...
|---|---|---|---|---|---|---|---|
| 2026-09-13 | The hands-free Lumi (`art-direction.md` §4a): the hub, a model sheet, and three loops — the wave again (24 frames, 120 ms), hands together (16, 200 ms, an idle variation), a book pick-up played back as the set-down (27 cells, 62 frames, 140 ms, a dev-only action) — tier 3 | 6 runs, $0.66 est.: the hub 1 run (4 of 4 passed), the model sheet 1 (1 of 2), the idle 1 (1 of 2), the wave 1 (1 of 2), the pick-up 2 (v1 kept three books on the stack; v2 1 of 3 chosen) | not yet reviewed | ~35 min from the first run (05:21) to wired and checked (05:50), with the pieces prototype built in parallel by a subagent | 1 new cut script + helper module, the preview script, 2 components, 8 docs | The cut, again, and the scripts: a ruled-line fix that also painted over a darkening ground and cut the figures in half (caught by uneven row scales), the grey-blue gap between her boots, and the cloak swapping shape where a loop from another sheet leaves the rest drawing | a drawn hub makes first-run passes the norm (4 of 5 sheets); with loops from several sheets, hold every one to one rest drawing and ease the joins; a prop fades in and out; the held-parts check caught nothing only because the holds were right, and cost nothing to run → stage 4, backlog, Prompt lab findings, `art/README.md`, `decisions.md` |
...
8. **Weight of the body sheet.** The hands-free sheet is 899 KB, twice the lantern's, most of it the pick-up's 27 cells. Before more actions: a sheet per action loaded when cued, or the action's rows at a lower WebP quality measured against the held-parts check. *Low because nothing ships until Chanté decides.*
...
**Next animation, not a backlog move:** wait for Chanté's verdict on the hands-free Lumi and the pieces test (`art-direction.md` §4a). If both are taken, the next path is one that ends in a room (setting the book on the Library's shelf), briefed from `art/prompts/lumi-free-pickup.md` with the hub as the reference; frequent small life goes to pieces, not sheets.
