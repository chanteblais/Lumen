...
**The served body is the hands-free Lumi** (since 2026-09-13): `scripts/cut-lumi-free.py` regenerates `public/lumi-free.webp` from one spec per loop in `LOOPS` (sheet, cells, region, rest cells, phase holds, monotone, ease, prop), with the helpers in `scripts/lumi_cut.py`; every loop is held to one rest drawing (the hands-free wave's first cell), its boots below the hem held in every cell, and the page draws her shadow. A new loop is a spec entry and a run. It is now the only cut that ships a sheet: the lantern cut (`scripts/cut-lumi-idle.py`, `public/lumi-idle.webp`, `public/lumi-heads.png`) was retired on 2026-09-15 (backlog 2, below). The steps below were learned on the lantern wave and are what the specs encode. A drawn loop goes through six steps, and the wave (2026-09-13) showed each one is needed — the last two cost a review round apiece when they were missing:
...
|---|---|---|---|---|---|---|---|
| 2026-09-15 | Not an animation: the avatar becomes one drawing (`lumi-avatar-head.png` → `public/lumi-avatar.webp`, `scripts/cut-lumi-avatar.py`) and the lantern cut is retired — tier 0 (a still) | 0 — Chanté drew it | — | ~35 min | 3 code + 7 docs | Choosing the framing: six candidates rendered on the forest disc at 36 · 48 · 68px before one was picked | Frame a still in the cut, not the component (the circle crops what the cut places); a preview on the disc at every size it ships at costs a minute and answers what a review round would → `art/README.md`, `design-system.md` → Lumi avatar |
...
2. **The cut takes a sheet spec, not code edits** — *half landed 2026-09-13 (`cut-lumi-free.py`), the rest is moot for now.* Every loop that ships is already a spec entry (`{sheet, cells, region, rest, holds, monotone, ease, prop}`) plus a run. What is left is the sheets nobody cuts today: a part-only sheet (the glance's `with_eyes`) has no spec, so the first one to come back costs its code again. *Saves* ~30 min on that sheet; ranked here because nothing is waiting on it.
...
### Landed
- 2026-09-15 — **The lantern cut retired** (backlog 2's second option: "retire `lumi-idle.webp` and its cut once nothing plays it"). Nothing had played the lantern body since the hands-free Lumi landed on 2026-09-13, and the avatar stopped being its six heads the same day it became one drawing (`scripts/cut-lumi-avatar.py`). Gone: `scripts/cut-lumi-idle.py` (its own copies of the matte, scale, place, align, breathe and lid helpers — `lumi_cut.py` is now the only set), `public/lumi-idle.webp` (488 KB) and `public/lumi-heads.png` (227 KB). *Saved*: 715 KB of served art, one cut to keep in step with every change to the helpers, and the question "which Lumi is this?" on every sheet touch. The source sheets stay in `art/lumi/`; the lantern sheet is still the style reference every prompt attaches.
