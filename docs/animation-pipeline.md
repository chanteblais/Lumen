...
**A sheet that only moves a part or two** (the foot play: the eyes and one boot) skips steps 4–5's per-sheet holds: its cells are aligned on everything that holds, then the rest cell takes only the moving parts from each (`with_eyes`, `with_boot` — where the two disagree, feathered). It stays one drawing with the breath and the wave even though the sheet is another generation, and a phase held on one cell (the eyes down) is a donor index, not code. Find eyes on the whole face and stop at the chin: eyes lowered to the ground cross the head rows.

The phase spans (`WAVE_REST`, `WAVE_WAVES`, `WAVE_RISE`, `WAVE_FALL`; `FOOT_REST`, `FOOT_EYES_DOWN`, `FOOT_PLAY`) and the regions (`HAND_ROWS`, `LOW_ROW`, `BOOT_TOP`, the held-parts weights) are per sheet today; backlog #2 moves them into a spec. Run with `--debug <dir>` for the body and heads on paper, and check the sheet size printed at the end.
...
|---|---|---|---|---|---|---|---|
| 2026-09-13 | The foot play (`lumi-foot-play.png`, 24 cells, 160 ms, four idle variations over one row: glance, scuff, foot, dawdle) — tier 3 | 1 run of 4 candidates (sunburst); 2 passed; Chanté chose the smoother from their motion GIFs | 0 before the review round (choosing between two passing sheets is not a send-back) | ~35 min (04:56 first run → 05:30 wired and checked), incl. landing backlog #1 | 2 scripts + 2 components + 8 docs | A false alarm: the new held-parts check read no eyes in 13 cells — the eye finder cut the face at the head rows before filling it, and eyes lowered to the ground cross that line (the cut's mask had the same cut, hidden by its 5px dilation); the measure script's *eyes dy* read the hood's embroidery and hid the look down | one sheet in phases that share poses gives several variations, orders over one row; take only the moving parts of a second generation onto the rest drawing (it holds by construction — bet 2's evidence again); a look up can be the look down replayed; find eyes on the whole face → stage 4–6, Prompt lab, backlog, `art/README.md`, `decisions.md` |
...
|---|---|---|---|
| 2026-09-13 | **Phases that share poses at their joins make one sheet several loops.** Asking for rest → look down → foot play → look up, each phase starting where the last ended, let the foot sheet play as a glance, one scuff, two, and a dawdle — four idle variations for one run. Naming the holds per phase (Prompt backlog 1a) held in one candidate of four; the joins mattered more. Also: sunburst held the drawing again (head IoU ≥ 0.955 in all four, ≥ 0.983 in the chosen one) with the wave's first cell as the reference. | lumi-foot-play · v1 · `sunburst-…-1` (chosen), `-4` | held once |
