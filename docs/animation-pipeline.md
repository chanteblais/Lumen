...
**Above #1 — objects as a template** (Chanté, 2026-09-13: Lumi will pick up and engage with lots of things; "I really wanna get this animation pipeline dialled"). Every object today costs a hand-written prompt, verdicts read by hand, a hand-edited cut spec and a zoomed look at the hands. Target: a path template (`art/prompts/` plus one object line: what, colours, where it stands, which hand, the held pose) and one command that generates candidates, prints pass or fail per gate (folding in #3 and #7), counts hands (dark blobs below the chin against the rest cell, so a third hand fails without a person looking), and writes the `LOOPS` spec from the prompt's phases. *Done when* the lantern pick-up goes from its object line to review with no other hand-edited file. Ranked first because every future object pays for it; plan in `art-direction.md` §4a, second round.
...
**Next animation, not a backlog move:** the pieces are approved (2026-09-13, "looks great") and the walk test is up for Chanté's eye (`art/prototypes/lumi-walk/`). If walking reads as her, movement in rooms is a rig, not sheets: the next steps are turns (a drawn in-between facing, or a squash that hides the swap), the base idle on the rig, and handling an object as a rig pose plus a short path — measured with `capture.py`'s frame grid, the way sheets are measured with the gates. If it reads as a puppet, the path that ends in a room (setting the book on the Library's shelf) is still briefed from `art/prompts/lumi-free-pickup.md`.
