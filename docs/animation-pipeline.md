...
`scripts/cut-lumi-idle.py` regenerates `public/lumi-idle.webp` and `public/lumi-heads.png`. The body comes from the wave sheet — every loop is one drawing, the breath synthesised from the wave's first cell — and the heads from the lantern sheet. A drawn loop goes through six steps, and the wave (2026-09-13) showed each one is needed — the last two cost a review round apiece when they were missing:
...
6. **Eye rows only where they show.** A short gesture keeps its eyes open (`LUMI_ROW_EYES`); blink rows for the wave's 24 cells tripled the sheet.

**A sheet that only moves a part or two** (the glance: the eyes) skips steps 4–5's per-sheet holds: its cells are aligned on everything that holds, then the rest cell takes only the moving part from each (`with_eyes`: both cells' eyes and glow, inside the face, feathered). It stays one drawing with the breath and the wave even though the sheet is another generation. **A part that moves must leave where it was.** Blending in only the pixels where the source differs from the rest cell draws the new part and keeps the old: the foot play's boot grew out of the rest boot and went back on review. For a limb, take the whole region — the old place and the new, dilated — from the source cell, then look at it at 3×. Find eyes on the whole face and stop at the chin: eyes lowered to the ground cross the head rows.
...
1. `src/components/chat/LumiSprite.tsx` — for a new sheet, `LUMI_ROWS` (the row, in sheet order) and `LUMI_ROW_EYES` (the eye rows it has), `LUMI_IDLE_FRAMES` if the row is the widest yet; for each loop over it, `LUMI_LOOPS` (the name), `LUMI_LOOP_ROW` (its row) and `LUMI_LOOP_CELLS` (the play order over the row's cells; repeats, reversals and several orders over one row are free); the header comment's row map.
2. `src/components/shell/LumiCompanion.tsx` — `FRAME_MS` (with the one-line reason), then `VARIATIONS` if it plays as an idle variation or `REACTIONS` if it plays in answer to something (with its trigger; backlog #5). The dev cue strip picks the new loop up on its own.
...
- The cut cells: `python3 scripts/preview-lumi-loop.py --row <sheet row> --cells <order> --ms <ms> --moving eyes,boot-left [--monotone 2-7:-:cloak-left] [--blink 4] --out <dir> --name <loop>` writes a contact strip of the loop in play order and a GIF of it as the page plays it (the fade in four steps a frame; `--blink` only for a row with eye rows), then the held-parts table and a PASS/FAIL line. Run it once per order when a row plays several ways. Look at the strip: the head must be one drawing across a held phase, the feet on one baseline, no cold rim, no grey fringe — and at 3× for anything small, since at 1× her lowered eyes looked gone when they weren't. Send the GIFs to Chanté (SendUserFile) for a first look before any server is up.
- **Held parts and coverage on the *cut* cells** (the preview's table since 2026-09-13): per cell against the rest cell, the feet, boots, lantern, hood edge and eyes within 0.5px; and, on alpha — lossless in the WebP, where colour thresholds read compression noise as movement — the moving side's coverage frame by frame, with no reversal inside a rise, a fall or a held phase. Both of the wave's review rounds were exactly these, and a strip at 1× hid both. Look at seams at 3×: a blend that brings back a hand shows there first.
...
|---|---|---|---|---|---|---|---|
| 2026-09-13 | The foot play (`lumi-foot-play.png`) → **the glance** (4 cells, 160 ms, an idle variation); its scuff, foot and dawdle orders were cut and dropped — tier 3 | 1 run of 4 candidates (sunburst); 2 passed; Chanté chose the smoother from their motion GIFs | 1 — "only glance really worked … it's just looking like a foot's growing out of her foot" | ~35 min to the first review (04:56 first run → 05:30 wired and checked), then the re-cut; incl. landing backlog #1 | 2 scripts + 2 components + 8 docs | The boot. It was cut by taking only the pixels that differed from the rest cell, which draws the new boot but never erases the old one. The preview printed the tell — the moving boot's coverage rose to 1.25× — and it was read as motion; the eyes were looked at at 3×, the boot only at 1×. Also a false alarm on the way: the eye finder cut the face at the head rows, so lowered eyes read as missing | a part that moves has to leave where it was — take its whole region from the source cell, never the difference; a moving part keeps its size, so coverage that grows is a warning (the preview prints it now); look at every moving part at 3×, not just the one that worries you; the variation that survived moved only the eyes, the cheapest part to move honestly (bet 1) → stages 4 and 6, Pitfalls, the preview script, `art/README.md`, `decisions.md` |
...
1. ~~**Held parts and coverage in the preview.**~~ Landed 2026-09-13 with the foot play (see *Landed*).
2. **The cut takes a sheet spec, not code edits.** Give `scripts/cut-lumi-idle.py` a list of specs — `{sheet, rows (by segmentation, landed for the wave), cells, place_by, held weights, phases: {rest, holds: [(span, donor, moving region)], monotone: [(span, direction, region)]}, eyes}` — so a new loop is one dict entry and one run. The wave's phase constants are the first spec in all but name. *Saves* ~30 min of per-sheet code; *done when* the wave re-cuts identically from its spec.
...
### Landed
- 2026-09-13 — **Held parts in the preview** (old backlog #1). `scripts/preview-lumi-loop.py` now measures every cell in play order against the first, on alpha, in regions found on the rest cell (hood, eyes, lantern, cloak-left, boot-left, boot-right): shift and IoU per region, `--moving` names what may move, `--monotone` flags coverage moving back inside a span, and a PASS/FAIL line ends it; `--row` is the sheet's row. Checked first on the approved wave (every held region within 0.01px) so the numbers meant something. On the foot play it verified every order before any server and caught a bug in the eye finder (lowered eyes read as missing). It did not stop a round: the scuffs held everything they should and still went back, because the moving boot was drawn over the rest boot. Its table printed the tell (the boot's coverage at 1.25×), and it now warns when a moving part's coverage leaves 0.8–1.2× of the first cell's.
...
|---|---|---|---|
| 2026-09-13 | **Phases that share poses at their joins let one sheet play several ways — if the cut can move the part.** Asking for rest → look down → foot play → look up, each phase starting where the last ended, gave a sheet the cut could order as a glance, one scuff, two and a dawdle. Only the glance survived review, and the prompt wasn't the cause: the sheet moves the boot correctly, the cut didn't (Pitfalls). Naming the holds per phase (Prompt backlog 1a) held in one candidate of four. Sunburst held the drawing again (head IoU ≥ 0.955 in all four, ≥ 0.983 in the chosen one) with the wave's first cell as the reference. | lumi-foot-play · v1 · `sunburst-…-1` (chosen), `-4` | held once |
...
- 2026-09-13 — The foot play's review round: only the glance kept. The ledger row now counts 1 round and what it taught; stage 4's parts-only paragraph and a new Pitfall say a moving part replaces its whole region; the preview warns when a moving part's coverage leaves 0.8–1.2× of the first cell's; the Landed and Prompt lab rows corrected.
- 2026-09-13 — The foot play: its ledger row (one run, two passes, Chanté chose the smoother; ~35 min to wired); backlog #1 landed (held parts in the preview, `--row` now the sheet row, `--moving`, `--monotone`); stage 4 gains the parts-only cut (`with_eyes`, `with_boot`) and "find eyes on the whole face"; stage 5 names `LUMI_ROWS`, `LUMI_ROW_EYES` and `LUMI_LOOP_ROW` (a row plays several ways); stage 6's command and a 3× look for small parts. Chanté lifted the image budget ("as much as you need") — the six-run stop now asks about tiers, not money.
