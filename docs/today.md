...
- **RIGHT NOW** — the dominant card, the only full paper on the page, no kicker inside: the title (display serif, large), a quiet marginal note beneath it in small caps (list · ~minutes; `.pill`, no box), after a *Not this*, Lumi's one line on why this fits instead (`plan.note`, italic), the first step in one line (*First* · …), then three quiet words: *Not this*, *Break it down*, *Done* — each happens on the card (On the card, below). No primary button since 2026-09-13 (*Start with Lumi* removed). No circle in front of the title — the card invites starting, not ticking. One card. No carousel, no "1 of 3". With nothing to choose from: *Nothing queued.* and one line.
...
## On the card (was: Handoffs into chat)
*2026-09-13, Chanté: "I'd really like to find a way to keep all that organizing on the Today page." Until then every control on the card sent you to Home as a chat message; now nothing does.*
- **Not this** → the six quick answers appear in place under "Fair. What's getting in the way?" (plus a quiet *Keep it*). The tapped answer goes to `PATCH /api/intentions/[id]` `{ action: "decline", reason }`: `intention.declined {reason}` is recorded and the path is re-cut at once (`reason: declined`) while the card reads *Finding something that fits…*; the page then shows the new Right now with Lumi's one line on why it fits (`plan.note` — the planner gets the declined title and reason as `justDeclined`; the note is dropped when the path advances). No chat message and no reply; Lumi sees *declined today (reason)* on the intention in her next turn. Resistance adapts the plan; it is never a failure.
