...
## Handoffs into chat
- **Start with Lumi** → `/` with a structured user message (`{ kind: "start_intention", intentionId }` in metadata; visible text "Let's start: *Finish discussion post*"). Lumi runs initiation and, once what · first step · how long are settled, opens a focus session (`start_focus_session`, M5) — the session bar appears above the composer; see `features.md` → Chat → Focus Together.
