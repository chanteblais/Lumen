...
## Lists (the pile)
*In the nav as the **Library** since 2026-09-13 (`/library`, `spaces.md`); the lists inside it are still lists, and this spec keeps the name. The same day the list view came off the page — the Library is only its painted room for now — so the rows below describe the data and the view as it was, not what the page shows.*
