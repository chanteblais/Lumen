...
| `docs/living/decisions.md` | `6eb0824e97e1` |
...
- **2026-09-18 · places: Today is the day at a glance** (`ux/today-at-a-glance`): the product decisions gained *Today is the day at a glance; there is no Right now card*. The places line said Today was "a path with one thing first" changed "on its card": with no card, she would have sent someone to look for one. Now it says what the page shows (each area's standing, a short suggested list in her order, what's fixed in time) and that changing it happens on a row. The planner still picks a first thing, so her order still means something; the brief doesn't claim more. Voice eval run logged in `voice-eval-log.md`. Re-stamped.
...
- **2026-09-13 · reviewed, nothing for Lumi** (merge of `main` into `fix/ai-and-routes`): the product decisions now hold both *Their word has to be theirs…* and *Today does its own organising…*, each already reviewed on its own branch; nothing new between them. Re-stamped; `brief.ts` is main's, byte for byte.
- **2026-09-13 · reviewed, nothing for Lumi** (`fix/ai-and-routes`): the product decisions gained *Their word has to be theirs, and only their word brings back what was forgotten* (code review B4). It's enforcement in code — a quote must carry what they meant, and only a checked quote brings back something forgotten — of what the brief already says: what they told you outranks what you inferred. `brief.ts` regenerated byte-identical.
