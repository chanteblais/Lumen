...
| `library.thread_created` / `.summary_revised` | `{ by: 'user'|'lumi'|'consolidation', their_word? }` — the subject is the thread; `their_word: true` when Lumi made it because they asked, in words the code found in their messages (`add_to_library`'s new thread, `shelve_thread`'s new section). Their word is a flag, never actor `user`, so the forgotten check still runs (code review B4, 2026-09-13) |
