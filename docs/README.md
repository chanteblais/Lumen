...
| Lumi's design digest (in the database, not a doc) | Lumi's own design notes from Chanté's design conversations and the daily digest of what changed: `npm run design:digest` (`-- --notebook` for every note with its history). Reference for design and implementation work — not canon, not requirements. [Architecture → Lumi's design notebook](architecture.md) |
