...
|---|---|---|---|---|
| 2026-09-13 | `scenery/library-background.png` | A lamplit library cutaway: a long reading table on a rug at its heart, shelves and a ladder, a gallery with a telescope, a curving stair, a window seat with a sleeping cat; no Lumi. Signs over its bays — *Personal*, *Coherence*, *School*, *Glåüm*, *Lumi's Notes*, *Archive*, *Ideas*, *Psychology* — and a banner, *More Thoughtful Humans* | served behind the Library (was Lists) as `public/library-room.webp`; Lumi's rest frame stands on the rug by the table, as on Today | its words: §3 rules 3–4 say no categories and no mottos in the painting, and the signs are both — they don't match the user's lists and nothing reads them; open for Chanté ([open question 3](living/open-questions.md)). `archived/library-background.png` is an earlier cut of the same room; `mockups/book-mockup.png` and `book_layout-mockup.png` (a thread as an open book) are not built |
