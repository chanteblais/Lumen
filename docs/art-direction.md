...
|---|---|---|---|---|
| 2026-09-13 | `scenery/library/background.png` | The blank-signs room repainted again: the same bays, boards, stove, gallery and stair, the shelves fuller (the centre bay's lower board gone for books), a moon-phase banner by the stair, the table and rug a little further back | the source for the Library; not yet served — `public/library-room.webp` is still cut from the repaint below until it is regenerated | whether Lumi's spot on the rug still lands in front of the table |
