...
| [Library](docs/product/lists-library.md) | What am I carrying, and where does it belong? | Browse and retrieve active context, relationships and history | `/library`, painted room with its sections (threads that hold threads) lettered on the bookcases, opening to shelves of books and a thread's book (pending review, `feat/library-sections`); the lists are browsed in **Lists**, a sheet from the nav over the page you're on (`/lists`), and still feed Lumi and Today |
