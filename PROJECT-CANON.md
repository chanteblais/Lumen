...
4. **Show less than you know; one decision at a time.** Disclose progressively. **State and shape may be shown at a glance** — where each area stands, how full the day is, when a rhythm happened — **counts, scores and anything the user has to interpret may not**: never a dashboard of metrics. Empty space is part of prioritisation. (V§11, EP§2–4, EP§12; [decision, 2026-09-18](docs/living/decisions.md) — until then this read "never build a dashboard")
...
- **Watch for drift:** the Garden becoming a scorecard (a count, a streak, a percentage — state at a glance is allowed, judgement is not); the Library needing grooming; Lumi turning into a generic chatbot or an authority; metrics added because apps have them; motivational copy; paintings becoming clutter; points standing in for growth; engagement over action. Each principle's *watch for* list is the detailed version. Flag drift. Don't refactor large areas without discussing first.
